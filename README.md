...
Gemini 2.5以降のモデルは **thinking**（内部推論）機能を持つ。thinkingが有効な場合、モデルは最終回答を生成する前に、ユーザには見えない内部推論トークンを生成する。これにより、複雑なタスクでの回答品質が向上する可能性がある。

- **`thinking_budget`**: 内部推論に使用できる最大トークン数。0に設定するとthinking機能が完全に無効化される
- thinking中に消費されるトークンは最終出力には含まれないが、API利用量（課金）には計上される
- `gemini-2.0-flash` はthinking機能を持たないため、`ThinkingConfig`自体を指定しない
...
| パラメータ | 値 | 説明 |
|---|---|---|
| `temperature` | 0.2 | 低めの温度で出力の再現性を高める |
